...
public:
    ...
    void solve(int nSteps = 10) {
        ...
        for (uint i = 0; i < nSteps; i++) {
            ...
            for(auto target : endEffectorTargets) {
                crl::Matrix dpdq;
                gcrr.estimate_linear_jacobian(target.p, target.rb, dpdq);
                crl::Matrix J = dpdq.block(0, 6, 3, deltaq.size()).eval();
                crl::Matrix J_T = J.transpose();
                crl::P3D p = target.target - gcrr.getWorldCoordinates(target.p, target.rb);
                Eigen::Vector3d v = Eigen::Vector3d(p.x, p.y, p.z);
                deltaq += (J_T * J).ldlt().solve(J_T * v);
            }
