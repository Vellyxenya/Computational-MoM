...
private:
    ...
    void generate(const bFrameState& startingbFrameState) {
        ...
        while (t < tEnd) {
            ...
            headingAngle = headingAngle + turningSpeed * dt; //headingAngle + atan2(turningSpeed, vForward) * dt; //headingAngle /* + TODO: fix this */;
            //auto a = heading * robot->forward;
            //std::cout << a.transpose() << std::endl;

            /**
             * Angular velocity is omega = dphi/dt (where phi is the angle)
             * and the so the variation of the angle is given by dphi = omega * dt.
             * if turningSpeed indicates an angular velocity, then dphi = turningSpeed * dt;
             * and hence headingAngle = headingAngle + dphi = headingAngle + turningSpeed * dt;
             */
