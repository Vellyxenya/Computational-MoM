...
P3D GeneralizedCoordinatesRobotRepresentation::
    ...
    P3D p_in_RB = pLocal;
    V3D v_in_RB = V3D(p_in_RB - getJointForQIdx(qIndex)->cJPos);
    V3D v_in_parent = rotateVec(v_in_RB, q[qIndex], getQAxis(qIndex));
    P3D p_in_parent = getJointForQIdx(qIndex)->pJPos + v_in_parent;
...
P3D GeneralizedCoordinatesRobotRepresentation::getWorldCoordinates(const P3D &p,
    ...
    P3D p_base = P3D(q[0], q[1], q[2]);
    V3D vec = rotateVec(rotateVec(rotateVec(V3D(pInWorld), q[3], getQAxis(3)), q[4], getQAxis(4)), q[5], getQAxis(5));
    pInWorld = p_base + vec;
...
void GeneralizedCoordinatesRobotRepresentation::compute_dpdq(const P3D &p,
    ...
    //We need the end effector coordinates for all columns:
    P3D e_pos = getWorldCoordinates(p, rb);

    //Jacobian for the 3 position coordinates
    dpdq.block(0, 0, 3, 3) = Matrix::Identity(3, 3);
    
    //Jacobian for the theta_j's:
    int qIndex = getQIdxForJoint(rb->pJoint);
    while(qIndex > 5) {
        V3D vec = V3D(e_pos - getJointForQIdx(qIndex)->getWorldPosition());
        V3D v = getQAxis(qIndex).cross(vec);
        dpdq(0, qIndex) = v[0];
        dpdq(1, qIndex) = v[1];
        dpdq(2, qIndex) = v[2];
        qIndex = getParentQIdxOf(qIndex);
    ...
    //Jacobian for the 3 orientations:
    P3D p_base = P3D(q[0], q[1], q[2]);
    V3D vec = V3D(e_pos - p_base);
    for(int i = 3; i < 6; i++) {
        V3D v = getQAxis(i).cross(vec);
        dpdq(0, i) = v[0];
        dpdq(1, i) = v[1];
        dpdq(2, i) = v[2];
    }
...
void GeneralizedCoordinatesRobotRepresentation::estimate_linear_jacobian(
    ...
    for (int i = 0; i < q.size(); i++) {
        ...
        V3D dpdq_i = V3D(p_p - p_m) / (2*h); //V3D dpdq_i(0, 0, 0);  // TODO: fix this: compute derivative dp(q)/dqi
