...
- Student ID: 18-804-880
- ETH Email: ngueddach@student.ethz.ch
- Demo: [link](https://drive.google.com/file/d/1aZu-lD-_FE9qTM9jpA6XQO9cI_JrUVFX/view?usp=sharing) (**Please note that for the the case when the restitution coefficient is set to 1.0, the video shows that the ball bounces off without touching the ground. This is just an artifact coming from the fact that the screen recorder seemingly does not record all the frames. To see the proper result, please run the code.**)
- Answer for Ex.1: 
   1. What do we notice? When we decrease the simulation step, the trajectory matches the analytical one more closely and when we increase, the simulated trajectory completely overshoots the analytical one (as well as making the simulation run faster, for obvious reasons).
   2. Why this happens? It is well-known that explicit integration schemes are unstable, meaning they introduce additional energy into the system, which explains why the cube flies further away.
