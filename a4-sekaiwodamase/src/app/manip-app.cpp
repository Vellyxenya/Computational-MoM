...
public:
	...
	virtual double evaluate(const VectorXd &u) const {
		...
		REGULARIZER reg = MAX_MEAN;

        if (QUESTION_12) {
            double totalEnergy = 0;
			auto triangles = sim->triangles;
			auto features = targetFeaturePoints;
			auto min_max = std::minmax_element(triangles.begin(), triangles.end(),
				[x](auto t1, auto t2) { return t1.energy(x) < t2.energy(x);});
			double min = min_max.first->energy(x);
			double max = min_max.second->energy(x);
			double mean = (max + min) / 2;
			
			if(reg == PINS_DIST_DIFF) {
				//Regularizer 1
				auto pins = sim->pins;
				double d1 = 2 * (pins[1].nodePos(0, x) - features[1]).norm();
				double d2 = 2 * (pins[5].nodePos(0, x) - features[1]).norm();
				totalEnergy += pow((d1 - d2), 2);
				d1 = 2 * (pins[0].nodePos(0, x) - features[0]).norm();
				d2 = 2 * (pins[4].nodePos(0, x) - features[0]).norm();
				totalEnergy += pow((d1 - d2), 2);
				O += 0.1 * totalEnergy;
			} else if (reg == PINS_DOT_ALIGNMENT) {
				//Regularizer 2
				if(O < 1e-2) {
					auto pins = sim->pins;
					Vector2d vvr = pins[1].nodePos(0, x) - pins[5].nodePos(0, x);
					Vector2d vvl = pins[0].nodePos(0, x) - pins[4].nodePos(0, x);
					Vector2d ffr = features[1] - pins[3].nodePos(0, x);
					Vector2d ffl = features[0] - pins[2].nodePos(0, x);
					double dotr = ffr.dot(vvr);
					double dotl = ffl.dot(vvl);
					if(dotr < 0) dotr *= -1;
					if(dotl < 0) dotl *= -1;
					totalEnergy += dotr + dotl;
					O += 0.1 * totalEnergy;
				}	
			} else if (reg == MAX_MEAN) {
				//Regularizer 3
				O += 10 * 1e-2 * pow((max - mean), 2);
			} else if (reg == MAX_MIN) {
				//Regularizer 4
				O += 1e-2 * pow((max - min), 2);
			} else {
				//Regularizer 5
				for(int i = 0; i < triangles.size(); i++) {
					double energy = triangles[i].energy(x);
					if(energy > mean) {}
						totalEnergy += energy;
				}
				O += 1e-5 * totalEnergy;
	...
	VectorXd get_DODu(const VectorXd &u) const {
		...
		VectorXd DODu; DODu.setZero(u.size()); {
			...
			if (step == FD_O_of_u) {
				...
				SparseMatrixd dFdx = -get_H(u, x); //F = - gradE => gradF = -Hess(E)
	...
	VectorXd get_dOdx(const VectorXd &x) const {
		...
		for (const int &i : I) {
			dOdx.segment<2>(2 * i) = seg2(x, i) - seg2(x_prime, i);
		}
		return dOdx;
	} 
		...
		SparseMatrixd ret; ret.resize(x.size(), u.size()); {
			for (const auto &pin : sim->pins) { 
				...
				MatrixXd dp_primedu; dp_primedu.setZero(2, u.size()); {
					...
					// => dp'dxy = ???
					dp_primedu.block<2, 2>(0, xy_dataStartIndex) << 1, 0, 0, 1; //Identity
					
					// => dp'dtheta = ???
					Matrix2d R_dot; //Derivative of 2D rotation matrix is just component-wise
									//derivative of its components
					R_dot << -sin(theta), -cos(theta), cos(theta), -sin(theta);
					dp_primedu.col(theta_dataIndex) = R_dot * f;
				} 
...
public: 
	void drawNanoVG() override { 
		...
		//for (const auto &pin : sim.pins     ) { drawAnchor(pin, sim.x); }
...
public: 
	...
	void drawAnchor(const Pin &pin, const VectorXd &x) { fillCircle(pin.anchorPosition, RED); } 
