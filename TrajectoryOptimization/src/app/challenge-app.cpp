...
public: 
	virtual double evaluate(const VectorXd &u_stack) const { 
		...
		double lambda_velocity = 4;
		
		double O = 0.;
		Vector2d entryPoint = x_sun + Vector2d(-1.5, 1).normalized() * r_prime;
		//End position constraint
		O += pow((x.back() - x_sun).norm() - r_prime, 2);
		//End velocity constraint
		/*double target_speed = 1/pow(r_prime, 1/2.);
		Vector2d vec = (x.back() - x_sun).normalized();
		Vector2d target_velocity = target_speed * Vector2d(-vec(1), vec(0)); //Change sign to choose one of the 2 orbit entries
		O += lambda_velocity * (target_velocity - v.back()).squaredNorm();*/
		
		double cumulated_error = 0;
		Vector2d x_simulate = x.back();
		Vector2d v_simulate = v.back();
		for(int i = 0; i < 40; i++) {
			auto xv_next = stepPhysicsSemiImplicitEuler(x_simulate, v_simulate, Vector2d::Zero());
			x_simulate = xv_next.first;
			v_simulate = xv_next.second;
			cumulated_error += pow((x_simulate - x_sun).norm() - r_prime, 2);
		}
		O += 0.3 * cumulated_error;
