...
public:
	...
	//coefficient for residual velocity penalty
	//Increase lambda_v to penalize non-zero end-velocity, since
	//it causes the spaceship to drift away when optimization is over.
	double lambda_v = 1;
	double lambda_xk = 1;
	double lambda_x0 = 10;
	double lambda_v0 = 10;
	double lambda_v_traj = 1;
	double lambda_u_traj = 1;
	...
	virtual double evaluate(const VectorXd &stack) const {
		...
		//End of trajecotry constraints
		O += lambda_xk * (x.back() - x_prime).squaredNorm() + lambda_v * v.back().squaredNorm();

		//Start of trajectory constraints
		O += lambda_x0 * (x.front() - x0).squaredNorm() + lambda_v0 * (v.front() - v0).squaredNorm();

		//Trajectory constraints
		double v_traj_objective = 0;
		double u_traj_objective = 0;
		for(int k = 0; k < K; k++) {
			v_traj_objective += (v[k] - (x[k+1] - x[k])/h).squaredNorm();
			u_traj_objective += (u[k] - (v[k+1] - v[k])/h).squaredNorm();
		}
		O += lambda_v_traj * v_traj_objective;
		O += lambda_u_traj * u_traj_objective;

		//regularizers
		if(SECOND_ORDER_SOLVER) {
			double reg = 0;
			for(int i = 0; i <= K; i++) {
				reg += u[i].squaredNorm() + 0.1 * v[i].squaredNorm();
				//we don't add a regularizer on x, because the trajectory should be translation-invariant
			}
			O += pow(10., log_c_reg) * reg;
		}
