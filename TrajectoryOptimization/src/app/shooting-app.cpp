...
Vector2d get_Fk(const Vector2d &uk, const Vector2d &xk) {
	...
	Fk += uk;
	//TODO probably apply gravity later on
	if(PLANET) {
...
public:
	...
	virtual double evaluate(const VectorXd &u_stack) const { 
		...
		// --
		double O = 0.;
		double lambda_v = 5; //coefficient for residual velocity penalty
		//Increase lambda_v to penalize non-zero end-velocity, since
		//it causes the spaceship to drift away when optimization is over.
		O += (x.back() - x_prime).squaredNorm() + lambda_v * v.back().squaredNorm();	
		//regularizer
		if(SECOND_ORDER_SOLVER) {
			double u_norm = 0;
			for(const auto u_i : u) {
				u_norm += u_i.squaredNorm();
			}
			O += pow(10., log_c_reg) * u_norm;
		}
		return O;
	}
};
